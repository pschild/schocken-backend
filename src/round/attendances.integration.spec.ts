...
describe('Attendances', () => {
  ...
  it('should be updated for an existing round', async () => {
    ...
    let result;
    result = await firstValueFrom(roundDetailService.updateAttendees(createdRound.round.id, { playerIds: [createdPlayer1.id, createdPlayer2.id] }));
    ...
    result = await firstValueFrom(roundDetailService.updateAttendees(createdRound.round.id, { playerIds: [createdPlayer2.id] }));
  ...
  it('should not be updated with an unknown playerId', async () => {
    ...
    await expect(firstValueFrom(roundDetailService.updateAttendees(createdRound.round.id, { playerIds: [RANDOM_UUID()] }))).rejects.toThrowError(/violates foreign key constraint/);
  });
  ...
  it('should not remove player when round is removed', async () => {
    ...
    const createdGame = await firstValueFrom(gameDetailService.create({ placeType: PlaceType.REMOTE }));
    const createdRound = await firstValueFrom(roundDetailService.create({ gameId: createdGame.id }));
    await firstValueFrom(roundDetailService.updateAttendees(createdRound.round.id, { playerIds: [createdPlayer.id] }));
  ...
  it('should not remove attendance if player is softly deleted', async () => {
    ...
    expect(result.attendees.length).toBe(1);
    expect(result.attendees[0].name).toEqual('John');
    expect(result.attendees[0].deletedDateTime).toBeNull();
    ...
    expect(result.attendees.length).toBe(1);
    expect(result.attendees[0].name).toEqual('John');
    expect(result.attendees[0].deletedDateTime).toBeDefined();
    expect(differenceInMilliseconds(new Date(), result.attendees[0].deletedDateTime)).toBeLessThan(1000);
